...
{
    internal class 前置_立刻突进 : Frost_WAR_ISlotResolver
    {
        public override int Check()
        {
            ...
            if(BattleData.当前目标 == null)
            {
                foreach (var keyValuePair in TargetMgr.Instance.EnemysIn20)
                {
                    ...
                    if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)WARActionID.猛攻, battleChara))
                    {
                        ...
                        target = battleChara;
                        Frost_WAR_RotationEntry.Frost_WAR_ArcUI.AddLog($"突进至{battleChara.Name}");
                        Core.Resolve<MemApiChatMessage>().Toast2($"突进至{battleChara.Name}", 1, 2000);
                        return base.Check();
                    }
                }
            }
            ...
            if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)WARActionID.猛攻, BattleData.当前目标))
            {
                ...
                Frost_WAR_RotationEntry.Frost_WAR_ArcUI.AddLog($"突进至{BattleData.当前目标.Name}");
                Core.Resolve<MemApiChatMessage>().Toast2($"突进至{BattleData.当前目标.Name}", 1, 2000);
                return base.Check();
            }
