...
{
    internal class 拉怪_无目标飞斧 : Frost_WAR_ISlotResolver
    {
        public override int Check()
        {
            ...
            if (BattleData.无目标的敌人列表.Count == 0)
            {
                return -3;
            }
            ...
            foreach (var enemy in BattleData.无目标的敌人列表)
            {
                if (GameObjectExtension.Distance(Core.Me, enemy, DistanceMode.IgnoreAll) <= 20
                    ...
                    && !Frost_WAR_DutyData.Instance.无法拉仇恨的敌人列表.Contains(enemy))
                {
