...
{
    internal class 自动雪仇_拉怪 : Frost_WAR_ISlotResolver
    {
        public override int Check()
        {
            ...
            int baseCheck = CanUseDefenceOGCD((uint)WARActionID.雪仇);
            if (baseCheck != 0)
            {
                return baseCheck;
            }
            ...
            if (Setting.自动减伤使用提示)
            {
                Core.Resolve<MemApiChatMessage>().Toast2($"雪仇已自动使用", 1, 2000);
            }
            return base.Check();
