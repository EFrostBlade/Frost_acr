...
{
    internal class 自动血气_受伤 : Frost_WAR_ISlotResolver
    {
        public override int Check()
        {
            ...
            if (Core.Me.Level >= 82)
            {
                ...
                if (Setting.自动减伤使用提示)
                {
                    Core.Resolve<MemApiChatMessage>().Toast2($"奶妈划水，原初的血气已自动使用", 1, 2000);
                }
                return base.Check();
            }
            else
            {
                ...
                if (Setting.自动减伤使用提示)
                {
                    Core.Resolve<MemApiChatMessage>().Toast2($"奶妈划水，原初的直觉已自动使用", 1, 2000);
                }
                return base.Check();
            }
        }
