...
{
    internal class 自动戮罪_死刑 : Frost_WAR_ISlotResolver
    {
        public override int Check()
        {
            ...
            if (Core.Me.Level >= 92)
            {
                ...
                if (Setting.自动减伤使用提示)
                {
                    Core.Resolve<MemApiChatMessage>().Toast2($"即将死刑，戮罪已自动使用", 1, 2000);
                }
                return base.Check();
            }
            else
            {
                ...
                if (Setting.自动减伤使用提示)
                {
                    Core.Resolve<MemApiChatMessage>().Toast2($"即将死刑，复仇已自动使用", 1, 2000);
                }
                return base.Check();
            }
        }
