...
{
    internal class BOSS中_战逃外葬送剑即将过期 : Frost_PLD_ISlotResolver
    {
        public override int Check()
        {
            ...
            {
                if (BattleData.葬送剑预备剩余时间 > 6)
                {
                    return -4;
                }
            }
            if (BattleData.葬送剑预备剩余时间 > 3)
            {
                return -5;
            }
            int baseCheck = CanUseGCD((uint)PLDActionID.葬送剑);
            if (baseCheck != 0)
            {
                return baseCheck;
            }
            SpellID = (uint)PLDActionID.葬送剑;
