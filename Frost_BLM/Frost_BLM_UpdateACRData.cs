...
using static Frost.Frost_BLM.Frost_BLM_Data.SpellUtils;
...
{
    internal class Frost_BLM_UpdateACRData
    {
        public static void UpdateBattleData()
        {
            ...
            battleData.战逃反应冷却时间 = SpellHelper.GetSpell((uint)BLMActionID.战逃反应).Cooldown.TotalSeconds;
            UpdateCombo();
            int targetMeCount = 0;
                ...
                if (battleChara != null)
                {
                    ...
                    if (GameObjectExtension.CanAttack(battleChara))
                    {
                        ...
                        else if (battleChara.TargetObjectId == myID)
                        {
                            ...
                            if (Frost_BLM_DutyData.Instance.无法拉仇恨的敌人列表.Contains(battleChara))
                            {
                                Frost_BLM_DutyData.Instance.无法拉仇恨的敌人列表.Remove(battleChara);
                            }
                        }
        ...
        public static void CalculateDamagePerSecond(LinkedList<(uint, long)> hpRecords, long currentTime)
        {
            ...
            Frost_BLM_BattleData.Instance.自身每秒承伤 = damagePerSecond;
        }
        private static void UpdateBattleState(IBattleChara? myTarget, int enemyIn25, int enemyIn5)
        {
            Frost_BLM_BattleData.Instance.上次战斗状态 = Frost_BLM_BattleData.Instance.当前战斗状态;
            var dutyData = Frost_BLM_DutyData.Instance;
        ...
        private static void UpdateBattleStateForNonBoss(int enemyIn25, int enemyIn5)
        {
            ...
            {
                case 4:
                    if ((enemyIn25 > enemyIn5 + Frost_BLM_DutyData.Instance.无法拉仇恨的敌人列表.Count() && Core.Resolve<MemApiMove>().IsMoving())
        ...
        public static void UpdateStatusState(IBattleChara? myTarget)
        {
            ...
            if (SpellHelper.GetSpell((uint)BLMActionID.预警).RecentlyUsed())
            {
                已有buff.Add("预警");
                自身承伤 *= (1 - 30 / 100.0);
                持有单减 += "预警" + ",";
            }
            if (SpellHelper.GetSpell((uint)BLMActionID.铁壁).RecentlyUsed())
            {
                已有buff.Add("铁壁");
                自身承伤 *= (1 - 20 / 100.0);
                持有单减 += "铁壁" + ",";
            }
            if (SpellHelper.GetSpell((uint)BLMActionID.雪仇).RecentlyUsed(3000))
            {
                目标buff.Add("雪仇");
                队伍承伤 *= (1 - 10 / 100.0);
                持有目标减 += "雪仇" + ",";
            }
            if (SpellHelper.GetSpell((uint)BLMActionID.亲疏自行).RecentlyUsed(3000))
            {
                目标buff.Add("减速");
                队伍承伤 *= (1 - 15 / 100.0);
                持有目标减 += "减速" + ",";
            }
            if (SpellHelper.GetSpell((uint)BLMActionID.极致防御).RecentlyUsed())
            {
                已有buff.Add("极致防御");
                自身承伤 *= (1 - 40 / 100.0);
                持有单减 += "极致防御" + ",";
            }
            else if (SpellHelper.GetSpell((uint)BLMActionID.壁垒).RecentlyUsed())
            {
                已有buff.Add("壁垒");
                自身承伤 *= (1 - 15 / 100.0);
                持有单减 += "壁垒" + ",";
            }
            if (SpellHelper.GetSpell((uint)BLMActionID.盾阵).RecentlyUsed())
            {
                已有buff.Add("盾阵");
                自身承伤 *= (1 - 15 / 100.0);
                持有单减 += "盾阵" + ",";
            }
            else if (SpellHelper.GetSpell((uint)BLMActionID.圣盾阵).RecentlyUsed())
            {
                已有buff.Add("圣盾阵");
                自身承伤 *= (1 - 15 / 100.0);
                持有单减 += "圣盾阵" + ",";
            ...
            if (SpellHelper.GetSpell((uint)BLMActionID.圣光幕帘).RecentlyUsed())
            {
                已有buff.Add("圣光幕帘");
                队伍承伤 *= (1 - 10 / 100.0);
                持有群减 += "圣光幕帘" + ",";
            }
            if (SpellHelper.GetSpell((uint)BLMActionID.武装戍卫).RecentlyUsed())
            {
                已有buff.Add("武装戍卫");
                队伍承伤 *= (1 - 15 / 100.0);
                持有群减 += "武装戍卫" + ",";
            ...
            for (int i = 0; i < Core.Me.StatusList.Length; i++)
            {
                ...
                if (!status.GameData.Value.Name.ToString().Contains("部队特效"))
                {
                    ...
                    if (status.StatusId == (uint)BLMBuffID.强化药)
                    {
                        battleData.爆发药剩余时间 = Math.Abs(status.RemainingTime);
                    }
                    if (status.StatusId == (uint)BLMBuffID.战逃反应)
                    {
                        battleData.战逃反应剩余时间 = Math.Abs(status.RemainingTime);
                    }
                    if (status.StatusId == (uint)BLMBuffID.安魂祈祷)
                    {
                        battleData.安魂祈祷剩余时间 = Math.Abs(status.RemainingTime);
                        battleData.安魂祈祷层数 = status.StackCount;
                    }
                    if (status.StatusId == (uint)BLMBuffID.悔罪预备)
                    {
                        battleData.悔罪预备剩余时间 = Math.Abs(status.RemainingTime);
                    }
                    if (status.StatusId == (uint)BLMBuffID.沥血剑预备)
                    {
                        battleData.沥血剑预备剩余时间 = Math.Abs(status.RemainingTime);
                    }
                    if (status.StatusId == (uint)BLMBuffID.赎罪剑预备)
                    {
                        battleData.赎罪剑预备剩余时间 = Math.Abs(status.RemainingTime);
                    }
                    if (status.StatusId == (uint)BLMBuffID.祈告剑预备)
                    {
                        battleData.祈告剑预备剩余时间 = Math.Abs(status.RemainingTime);
                    }
                    if (status.StatusId == (uint)BLMBuffID.葬送剑预备)
                    {
                        battleData.葬送剑预备剩余时间 = Math.Abs(status.RemainingTime);
                    }
                    if (status.StatusId == (uint)BLMBuffID.神圣魔法效果提高)
                    {
                        battleData.神圣魔法效果提高剩余时间 = Math.Abs(status.RemainingTime);
                    }
                    if (status.StatusId == (uint)BLMBuffID.神圣领域)
                    {
                        battleData.神圣领域剩余时间 = Math.Abs(status.RemainingTime);
                    }
                    else if (status.StatusId == (uint)BLMBuffID.钢铁信念)
                    {
                        是否开盾 = true;
                        battleData.是否开盾 = true;
                    }
                    else if (status.StatusId == (uint)BLMBuffID.战技封印1)
                    {
                        是否战技封印 = true;
                        battleData.是否战技封印 = true;
                    }
                    else if (status.StatusId == (uint)BLMBuffID.战技封印2)
                    {
                        是否战技封印 = true;
                        battleData.是否战技封印 = true;
                    }
                    else if (status.StatusId == (uint)BLMBuffID.能力封印1)
                    {
                        是否能力封印 = true;
                        battleData.是否能力封印 = true;
                    }
                    else if (status.StatusId == (uint)BLMBuffID.能力封印2)
                    {
                        是否能力封印 = true;
                        battleData.是否能力封印 = true;
                    }
                    else if (status.StatusId == (uint)BLMBuffID.能力封印3)
                    {
                        是否能力封印 = true;
                        battleData.是否能力封印 = true;
                    }
        ...
        public static void ModifySlotResolvers(List<SlotResolverData> slotResolvers)
        {
            ...
            if (QT.GetQt("停手"))
            {
                //Frost_BLM_RotationEntry.Frost_BLM_ArcUI.AddLog("切换至停手");
            ...
            else
            {
                ...
                if (QT.GetQt("泄资源"))
                {
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_泄资源_Defence_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_泄资源_OGCD_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_泄资源_GCD_SlotResolvers(), SlotMode.Gcd));
                    //Frost_BLM_RotationEntry.Frost_BLM_ArcUI.AddLog("切换至泄资源状态");
                ...
                {
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_拉怪_Defence_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_拉怪_OGCD_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_拉怪_GCD_SlotResolvers(), SlotMode.Gcd));
                ...
                {
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_小怪战斗即将结束_Defence_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_小怪战斗即将结束_OGCD_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_小怪战斗即将结束_GCD_SlotResolvers(), SlotMode.Gcd));
                    //Frost_BLM_RotationEntry.Frost_BLM_ArcUI.AddLog("切换至小怪战斗即将结束状态");
                ...
                {
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_小怪战斗中_Defence_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_小怪战斗中_OGCD_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_小怪战斗中_GCD_SlotResolvers(), SlotMode.Gcd));
                ...
                {
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_BOSS战斗中_Defence_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_BOSS战斗中_OGCD_SlotResolvers(), SlotMode.OffGcd));
                    //tempSlotResolver.Add(new SlotResolverData(new Frost_BLM_BOSS战斗中_GCD_SlotResolvers(), SlotMode.Gcd));
                    //Frost_BLM_RotationEntry.Frost_BLM_ArcUI.AddLog("切换至BOSS战斗中状态");
                    battleData.ResolverState = 5;
                }
            }
            */
                ...
                if (Core.Me.TargetObject != null)
                {
                    ...
                    if (target != null)
                    {
                        if (GameObjectExtension.CanAttack(target))
                        {
                            ...
                            {
                                if (CanUseOGCD_NoCombat((uint)BLMActionID.先锋剑))
                                {
                                    await SpellHelper.GetSpell((uint)BLMActionID.先锋剑, SpellTargetType.Target).Cast();
                                }
                            }
                            else if (GameObjectExtension.Distance(Core.Me, Core.Me.GetCurrTarget(), DistanceMode.IgnoreAll) <= 20)
                            {
                                if (CanUseOGCD_NoCombat((uint)BLMActionID.投盾))
                                {
                                    if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)BLMActionID.投盾, target))
                                    {
                                        await SpellHelper.GetSpell((uint)BLMActionID.投盾, SpellTargetType.Target).Cast();
                                    }
                                }
                            }
                        }
        ...
        public static async Task SpellCast()
        {
            ...
            if (Core.Resolve<MemApiCondition>().IsBoundByDuty()
                ...
                && Frost_BLM_Settings.Instance.自动疾跑)
            {
                if (Frost_BLM_DutyData.Instance.副本人数 == 4 || Frost_BLM_DutyData.Instance.副本人数 == 24)
                {
                    if (DateTime.Now - Frost_BLM_DutyData.Instance.上次疾跑时间 > TimeSpan.FromSeconds(60))
                    {
                        if (CanUseOGCD_NoCombat((uint)BLMActionID.冲刺))
                        {
                            await SpellHelper.GetSpell((uint)BLMActionID.冲刺).Cast();
            ...
            if (SC.GetSC("神圣领域"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.神圣领域))
                {
                    await SpellHelper.GetSpell((uint)BLMActionID.神圣领域).Cast();
                }
            }
            if (SC.GetSC("极致防御"))
            {
                if (Core.Me.Level >= 92)
                {
                    if (CanUseOGCD_NoCombat((uint)BLMActionID.极致防御))
                    {
                        await SpellHelper.GetSpell((uint)BLMActionID.极致防御).Cast();
                    }
                }
                else
                {
                    if (CanUseOGCD_NoCombat((uint)BLMActionID.预警))
                    {
                        await SpellHelper.GetSpell((uint)BLMActionID.预警).Cast();
                    }
                }
            }
            if (SC.GetSC("铁壁"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.铁壁))
                {
                    await SpellHelper.GetSpell((uint)BLMActionID.铁壁).Cast();
                }
            }
            if (SC.GetSC("壁垒"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.壁垒))
                {
                    await SpellHelper.GetSpell((uint)BLMActionID.壁垒).Cast();
                }
            }
            if (SC.GetSC("圣盾阵"))
            {
                if (Core.Me.Level >= 82)
                {
                    if (CanUseOGCD_NoCombat((uint)BLMActionID.圣盾阵))
                    {
                        await SpellHelper.GetSpell((uint)BLMActionID.圣盾阵).Cast();
                    }
                }
                else
                {
                    if (CanUseOGCD_NoCombat((uint)BLMActionID.盾阵))
                    {
                        await SpellHelper.GetSpell((uint)BLMActionID.盾阵).Cast();
                    }
                }
            }
            if (SC.GetSC("雪仇"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.雪仇))
                {
                    await SpellHelper.GetSpell((uint)BLMActionID.雪仇).Cast();
                }
            }
            if (SC.GetSC("圣光幕帘"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.圣光幕帘))
                {
                    await SpellHelper.GetSpell((uint)BLMActionID.圣光幕帘).Cast();
                }
            }
            if (SC.GetSC("亲疏自行"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.亲疏自行))
                {
                    await SpellHelper.GetSpell((uint)BLMActionID.亲疏自行).Cast();
                }
            }
            if (SC.GetSC("武装戍卫"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.武装戍卫))
                {
                    ...
                    Frost_BLM_RotationEntry.Frost_BLM_ArcUI.AddLog(log);
                    ...
                    await SpellHelper.GetSpell((uint)BLMActionID.武装戍卫).Cast();
                }
            }
            if (SC.GetSC("调停"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.调停))
                {
                    ...
                    if (_target != null)
                    {
                        ...
                        {
                            if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)BLMActionID.调停, _target))
                            {
                                await SpellHelper.GetSpell((uint)BLMActionID.调停, _target).Cast();
                            }
                        }
                    }
                }
            ...
            if (SC.GetSC("下踢"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.下踢))
                {
                    ...
                    if (_target != null)
                    {
                        ...
                        {
                            if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)BLMActionID.下踢, _target))
                            {
                                await SpellHelper.GetSpell((uint)BLMActionID.下踢, _target).Cast();
                            }
                        }
                    }
                }
            ...
            if (SC.GetSC("插言"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.插言))
                {
                    ...
                    if (_target != null)
                    {
                        if (_target.DistanceToPlayer() <= (float)SettingMgr.GetSetting<GeneralSettings>().AttackRange)
                        {
                            if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)BLMActionID.插言, _target))
                            {
                                await SpellHelper.GetSpell((uint)BLMActionID.插言, _target).Cast();
                            }
                        }
                    }
                }
            ...
            if (SC.GetSC("挑衅"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.挑衅))
                {
                    ...
                    if (_target != null)
                    {
                        ...
                        {
                            if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)BLMActionID.挑衅, _target))
                            {
                                await SpellHelper.GetSpell((uint)BLMActionID.挑衅, _target).Cast();
                            }
                        }
                    }
                }
            ...
            if (SC.GetSC("退避"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.退避))
                {
                    ...
                    if (_target != null)
                    {
                        if (_target.DistanceToPlayer() <= 25)
                        {
                            if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)BLMActionID.退避, _target))
                            {
                                await SpellHelper.GetSpell((uint)BLMActionID.退避, _target).Cast();
                            }
                        }
                    }
                }
            ...
            if (SC.GetSC("干预"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.干预))
                {
                    ...
                    if (_target != null)
                    {
                        ...
                        {
                            if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)BLMActionID.干预, _target))
                            {
                                await SpellHelper.GetSpell((uint)BLMActionID.干预, _target).Cast();
                            }
                        }
                    }
                }
            ...
            if (SC.GetSC("保护"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.保护))
                {
                    ...
                    if (_target != null)
                    {
                        if (_target.DistanceToPlayer() <= 20)
                        {
                            if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)BLMActionID.保护, _target))
                            {
                                await SpellHelper.GetSpell((uint)BLMActionID.保护, _target).Cast();
                            }
                        }
                    }
                }
            ...
            if (SC.GetSC("战逃反应"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.战逃反应))
                {
                    await SpellHelper.GetSpell((uint)BLMActionID.战逃反应).Cast();

                }
            }
            if (SC.GetSC("深仁厚泽"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.深仁厚泽))
                {
                    ...
                    if (_target != null)
                    {
                        if (_target.DistanceToPlayer() <= 30)
                        {
                            if (Core.Resolve<MemApiSpell>().CheckActionInRangeOrLoS((uint)BLMActionID.深仁厚泽, _target))
                            {
                                await SpellHelper.GetSpell((uint)BLMActionID.深仁厚泽, _target).Cast();
                            }
                        }
                    }
                }
            ...
            if (SC.GetSC("钢铁信念"))
            {
                if (CanUseOGCD_NoCombat((uint)BLMActionID.钢铁信念))
                {
                    await SpellHelper.GetSpell((uint)BLMActionID.钢铁信念).Cast();
                }
            }
            if (SC.GetSC("冲刺"))
            {
