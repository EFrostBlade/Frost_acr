...
public class Frost_BLM_RotationEntry : IRotationEntry
{

    public string AuthorName { get; set; } = "Frost";
    public static IDalamudPluginInterface PluginInterface =>Svc.PluginInterface;

    internal static NavmeshIpc navmesh;

    public Rotation Build(string settingFolder)
    {
        // 初始化设置
        Frost_BLM_Settings.Build(settingFolder);
        // 初始化QT （依赖了设置的数据）
        BuildQT();
        BuildSC();
        ...
        // 添加各种事件回调
        rot.SetRotationEventHandler(new Frost_BLM_RotationEventHandler());
        // 添加QT开关的时间轴行为
        rot.AddTriggerAction();
        rot.AddTriggerCondition();
        LogHelper.Info("Frost_BLM_RotationEntry初始化完成");
        navmesh = new NavmeshIpc(PluginInterface);
        LogHelper.Info("Navmesh加载成功");
        return rot;
    }
    ...
    public static Frost_BLM_AcrUi Frost_BLM_ArcUI = new();
    ...
    public void BuildQT()
    {
        ...
        Frost_BLM_RotationEntry.JobViewWindow = new JobViewWindow(Frost_BLM_Settings.Instance.JobViewSave, Frost_BLM_Settings.Instance.Save, "Frost 骑士");
        //Frost_BLM_RotationEntry.JobViewWindow.SetUpdateAction(OnUIUpdate); // 设置QT中的Update回调 不需要就不设置

        //添加QT分页 第一个参数是分页标题 第二个是分页里的内容
        Frost_BLM_RotationEntry.JobViewWindow.AddTab("通用", new Action<JobViewWindow>(Frost_BLM_ArcUI.DrawGeneral));
        Frost_BLM_RotationEntry.JobViewWindow.AddTab("Dev", new Action<JobViewWindow>(Frost_BLM_ArcUI.DrawDev));
        Frost_BLM_RotationEntry.JobViewWindow.AddTab("更新说明与反馈", new Action<JobViewWindow>(Frost_BLM_ArcUI.DrawUpdate));

        void AddQt(string name, bool qtValueDefault, string toolTip)
        {
            Action<bool> action = (value) =>
            {
                Frost_BLM_RotationEntry.Frost_BLM_ArcUI.AddLog($"QT被点击: {name} = {value}");
                if (name == "突进无位移")
                {
                    if (value == false)
                    {
            ...
            Frost_BLM_RotationEntry.JobViewWindow.AddQt(name, qtValueDefault, action);
            Frost_BLM_RotationEntry.JobViewWindow.SetQtToolTip(toolTip);
        }
        ...
        // 添加快捷按钮 (带技能图标)
        Frost_BLM_RotationEntry.JobViewWindow.AddHotkey("爆发药", new HotKeyResolver_Potion());
        Frost_BLM_RotationEntry.JobViewWindow.AddHotkey("极限技", new HotKeyResolver_LB());
    ...
    public void BuildSC()
    {
        Frost_BLM_Settings Setting = Frost_BLM_Settings.Instance;
    ...
    {
        Frost_BLM_SettingUI.Instance.Draw();
    }
    ...
    {
        return Frost_BLM_RotationEntry.JobViewWindow;
    }
    public void Dispose()
    {
        // 释放需要释放的东西 没有就留空
        Frost_BLM_RotationEventHandler.Framework.Update -= Frost_BLM_RotationEventHandler.UpdateACR;
    }
}
