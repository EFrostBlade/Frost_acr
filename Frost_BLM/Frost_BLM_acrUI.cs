...
using static Frost.Frost_BLM.Frost_BLM_RotationEventHandler;
...
{
    public class Frost_BLM_AcrUi : IRotationUI
    {
        public static Frost_BLM_AcrUi UI { get; private set; }
        ...
        {
            Frost_BLM_SettingUI.DrawSettings();
        }
        static Frost_BLM_AcrUi()
        {
            PostFormRequest();
        }
        ...
        public void DrawDev(JobViewWindow JobViewWindow)
        {
            ...
            {
                var target = Core.Me.TargetObject;
                if (target != null)
                {
                    var targetPos = target.Position;
                    var currentPos = Core.Me.Position;

                    // 创建一个取消令牌（根据需要你也可以直接使用 CancellationToken.None）
                    CancellationTokenSource cts = new CancellationTokenSource();

                    // 异步计算路径（这里假设不允许飞行）
                    Frost_BLM_RotationEntry.navmesh.Pathfind(currentPos, targetPos, false, cts.Token)
                        .ContinueWith(task =>
                        {
                            if (task.Status == TaskStatus.RanToCompletion)
                            {
                                ...
                                // 调用移动方法
                                Frost_BLM_RotationEntry.navmesh.MoveTo(path, false);
                                LogHelper.Print($"尝试移动到目标位置：{targetPos}");
                            }
                            else
                            {
                                LogHelper.Print("路径计算失败");
                            }
                        });
                }
            }
            foreach (var obj in Svc.Objects)
            {
                if (obj != null)
                {
                    ...
                    {
                        var objb = obj as IBattleChara;

                        if (objb != null)
                        {
                            var currentPos = Core.Me.Position;
                            var distance = Vector3.Distance(currentPos, objPos);
                            ImGui.Text($"玩家ID:{obj.GameObjectId},职业:{(objb.ClassJob.Value.Name)},血量{objb.CurrentHp}/{objb.MaxHp},可选中{objb.IsTargetable},可攻击{objb.CanAttack()} 名称: {obj.Name}, 位置: {objPos},距离:{distance}");
                        }
                        else
                        {
                            ImGui.Text($"玩家转换失败ID: {obj.GameObjectId},可选中{obj.IsTargetable},名称: {obj.Name}, 位置: {objPos}");
                        }
                    }
                    else if (obj.ObjectKind == ObjectKind.BattleNpc)
                    {
                        var objb = obj as IBattleNpc;
                        if (objb != null)
                        {
                            var currentPos = Core.Me.Position;
                            var distance = Vector3.Distance(currentPos, objPos);
                            ImGui.Text($"NPC ID:{obj.GameObjectId},职业:{(objb.ClassJob.Value.Name)},血量{objb.CurrentHp}/{objb.MaxHp},可选中{objb.IsTargetable},可攻击{objb.CanAttack()} 名称: {obj.Name}, 位置: {objPos},距离:{distance}");
                        }
                        else
                        {
                            ImGui.Text($"NPC转换失败ID: {obj.GameObjectId},可选中{obj.IsTargetable},名称: {obj.Name}, 位置: {objPos}");
                        }
                    }
                }
            }
        ...
        {
            if (Enum.IsDefined(typeof(BLMActionID), skillId))
            {
                return ((BLMActionID)skillId).ToString();
            }
            return "未知";
        }
        ...
        public void PostFeedback(string email, string issueDescription)
        {
            Task.Run(async () =>
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    Frost_BLM_RotationEntry.Frost_BLM_ArcUI.AddLog($"{responseBody}");
        ...
        public static implicit operator Frost_BLM_AcrUi(JobViewWindow v)
        {
